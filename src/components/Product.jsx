...
export default function Product({onIncrease, onDecrease, count, onCart}) {
  ...
  return (
    <section className="p-6 mb-10 xl:grid xl:place-content-center">
      ...
      <h1 className="font-700 text-3xl mb-3 xl:text-5xl xl:mb-10 xl:col-span-2">{products.product_name}</h1>
      ...
      <div id="price" className="flex justify-between items-center mb-5 xl:flex-col xl:items-start xl:gap-3 xl:col-span-2 xl:mb-9">
        <div id="discount-price" className="flex items-center gap-4">
          <h2 className="font-700 text-2xl xl:text-3xl">${products.final_price}</h2>
          <span className="font-700 text-sm bg-secondary text-primary px-2 py-0.5 rounded-md">
            {products.discount}%
          </span>
        </div>
        <div id="original-price">
          <p className="font-700 line-through text-sm text-gray-400 xl:text-base">${products.price_before}</p>
        </div>
      </div>
      ...
      <Button title={'Add To Cart'} onCart = {()=>onCart(products)}/>
